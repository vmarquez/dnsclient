...
object CodecLenses {
  ...
  /* ***************** */
  implicit def datagramIso: Iso[Err \/ (InetSocketAddress, Array[Byte]), Err \/ DatagramPacket] = ???// Iso((t) => new DatagramPacket(Unpooled.copiedBuffer(t._2), t._1), dgp => (dgp.recipient, dgp.content.array))
  ...
  implicit def bvToBa: Iso[Err \/ BitVector, Err \/ Array[Byte]] = Iso(bv => bv.map(b => b.toByteArray), ba => ba.map(b => ByteVector(b).toBitVector))
   ...
   def codecToBytes: Iso[Err \/ A, Err \/ Array[Byte]] = codecIso compose bvToBa 
}
